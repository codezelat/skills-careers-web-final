...
function DeleteConfirmation({ pressrelease, onClose }) {
  ...
  const handleDelete = async () => {
    try {
      ...
      const response = await fetch(
        `/api/pressrelease/delete?id=${pressrelease._id}`,
        {
          method: "DELETE",
        }
      );
      ...
      if (!response.ok) {
        alert("Failed to delete Pressrelease");
      }
    } catch (error) {
      console.error("Error deleting Pressrelease:", error);
      alert("Error deleting Pressrelease");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="absolute top-2/4 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10 p-8 bg-white rounded-xl shadow-xl text-center">
      <h3 className="text-base uppercase font-bold text-red-500 mb-4">
        Delete <span className="text-black">{pressrelease.title}</span> Press
        Release
      </h3>
