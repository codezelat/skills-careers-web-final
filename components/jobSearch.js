                ...
                const jobsWithRecruiters = await Promise.all(
                    jobsData.jobs.map(async (job) => {
                        try {
                            ...
                            if (!recruiterResponse.ok) throw new Error("Recruiter fetch failed");
                            const recruiterData = await recruiterResponse.json();
                            return { ...job, recruiter: recruiterData };
                        } catch (error) {
                            console.error(`Error fetching recruiter for job ${job.jobId}:`, error);
    ...
    return (
        <div className="flex flex-col relative mt-10">
            ...
            <div className="absolute top-full left-0 right-0">
                        ...
                        {jobSuggestions.map((job, index) => (
                            ...
                            >
                                ...
                                <div className="flex flex-col border-l-2 pl-6 gap-1">
                                    ...
                                    <p className="text-sm font-semibold">{job.recruiter.recruiterName} | {job.location}</p>
                                </div>
                ...
                {(!jobLoading && searchQuery.length >= 2 && jobSuggestions.length === 0) && (
