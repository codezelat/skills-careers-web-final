...
export default function AllRecruiterData({
  recruiter,
  isSelected,
  onSelect,
}) {
  ...
  return (
    <div className="p-4 rounded-lg transition-shadow border-b border-gray-200 flex items-center">
     <div className="flex flex-row space-x-4 w-1/3 items-center">
      ...
      {/* Website */}
      <div className="w-1/4 font-medium items-center">{contactNumber}</div>
      {/* Actions */}
      <div className="w-1/3 flex justify-end gap-2">
        <Link href="/admin/recruiters/editProfile">
          <button className="bg-[#001571] text-white px-5 py-2 rounded-lg text-sm flex items-center space-x-2">
            <Image
